...
// The Kiro translator stopped emitting a top-level `systemPrompt` (the gateway
// rejects the field with 400 REQUEST_BODY_INVALID); the prompt rides in the
// first user turn's content now. Read it from there.
const systemPromptOf = (out) => {
  ...
  const firstUser = Array.isArray(history)
    ...
    : null;
  return firstUser?.content ?? out?.conversationState?.currentMessage?.userInputMessage?.content ?? "";
};
...
describe("Claude → Kiro (direct route)", () => {
  ...
  it("carries a Claude system block into user content without a top-level systemPrompt", () => {
    const out = C2K({
      system: "system-only instruction",
      messages: [{ role: "user", content: "hello" }],
    });

    // A top-level systemPrompt is rejected by the gateway, so the field must be
    // absent and the instruction must reach the user turn instead.
  ...
  it("keeps the system prompt stable across turns while the time context moves", () => {
    ...
    expect(systemPromptOf(second)).toContain("stable instruction");
