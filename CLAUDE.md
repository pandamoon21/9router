...
cp .env.example .env
npm install
npm run dev                      # dev server on 20127 (webpack)
npm run build && npm run start   # production: build, then custom-server.js on 20127
```
- Bun variants: `npm run dev:bun` / `build:bun` / `start:bun`.
- **Two ports, and the script wins.** The `dev`/`start` scripts hardcode `--port 20127`, so a repo checkout serves **20127** regardless of `PORT` in `.env`. **20128** is the value baked into the Docker image, `docker-compose.yml`, and the published CLI (`cli/cli.js` `DEFAULT_PORT`, `cli/src/cli/api/client.js`). `PORT` in `.env.example` is for those deployment paths, not for `npm run {dev,start}`. Dashboard at `/dashboard`, API at `/v1`.
- `npm run start` runs **`custom-server.js`**, not `next start` — see the IP-sanitizing wrapper note under Conventions. A bare `next start` skips that security layer.
...
>
> **The suite is NOT expected to be all-green on a plain checkout.** Judge regressions with `tests/__baseline__/verify-no-regression.mjs`, not a raw run. Expected red:
> - 117 entries in `tests/__baseline__/known-fails.txt` (regenerated after the upstream v0.5.75 merge; the old count of ~26 is stale). This file is the gate's allowlist — a raw test run's failures are only meaningful against it.
...
- `tests/vitest.config.js` resolves `open-sse`/`@/` aliases from the repo root, so vitest may be run from `tests/` or with that config — but it is **not** wired into root `npm test` (root has no test script).
